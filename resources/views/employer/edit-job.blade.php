...
@section('title', 'Sửa tin tuyển dụng | MyJobPoly')
...
<div class="container account-grid">
    ...
    <main class="account-content">
        <h1 class="account-content-title">✏️ Sửa tin tuyển dụng</h1>
        <p class="account-content-subtitle">Cập nhật thông tin bài đăng tuyển dụng của bạn</p>
        ...
        <form action="{{ route('employer.jobs.update', $job->id) }}" method="POST" id="edit-job-form">
            @csrf
            @method('PUT')
            ...
            <div class="employer-form-section">
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:16px;">
                    ...
                    <div class="form-group">
                        ...
                        <select name="experience" class="form-control" required>
                            ...
                            @foreach(['Không yêu cầu', 'Dưới 1 năm', '1 - 2 năm', '2 - 3 năm', '3 - 5 năm', 'Trên 5 năm'] as $exp)
                                <option value="{{ $exp }}"
                                    {{ old('experience', $job->experience) == $exp ? 'selected' : '' }}>
                                    {{ $exp }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" name="deadline" class="form-control"
                               value="{{ old('deadline', \Carbon\Carbon::parse($job->deadline)->format('Y-m-d')) }}" required>
                    </div>
                </div>
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:16px; margin-top:16px;">
                    <div class="form-group">
                        ...
                        <select name="degree" class="form-control">
                            ...
                            @foreach(['Trung cấp', 'Cao đẳng', 'Đại học', 'Thạc sĩ/Tiến sĩ'] as $deg)
                                <option value="{{ $deg }}" {{ old('degree', $job->degree) == $deg ? 'selected' : '' }}>{{ $deg }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="quantity" class="form-control" value="{{ old('quantity', $job->quantity ?? 1) }}" min="1">
                    </div>
                </div>
            </div>
            ...
            <div class="employer-form-section">
                ...
                <div class="form-group">
                    ...
                    <textarea name="description" class="form-control" rows="6"
                              placeholder="Mô tả chi tiết về vị trí, trách nhiệm công việc..." required>{{ old('description', $job->description) }}</textarea>
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea name="requirements" class="form-control" rows="5"
                              placeholder="Yêu cầu kỹ năng, kinh nghiệm, bằng cấp..." required>{{ old('requirements', $job->requirements) }}</textarea>
                </div>

                <div class="form-group">
                    <label class="form-label">Quyền lợi</label>
                    <textarea name="benefits" class="form-control" rows="4"
                              placeholder="Các phúc lợi, chế độ đãi ngộ dành cho ứng viên...">{{ old('benefits', $job->benefits) }}</textarea>
                </div>
            </div>

            {{-- SUBMIT --}}
            <div style="display:flex; gap:12px; justify-content:flex-end; margin-top:24px;">
                <a href="{{ route('employer.jobs.index') }}" class="btn btn-outline" style="padding:12px 24px;">Hủy</a>
